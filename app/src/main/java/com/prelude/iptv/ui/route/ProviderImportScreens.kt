 ...
 *
 * ΠΡΟΣΟΧΗ — ΑΥΤΗ ΤΗ ΣΤΙΓΜΗ ΔΕΝ ΤΟΝ ΚΑΛΕΙ ΚΑΝΕΙΣ.
 *
 * Το μοναδικό του σημείο εισόδου ήταν το `AddMenuSheet`, το φύλλο που άνοιγε το
 * «＋» της κάτω μπάρας. Το φύλλο αφαιρέθηκε κατόπιν αιτήματος του κατόχου, ώστε
 * το «＋» να πηγαίνει κατευθείαν στην προσθήκη πηγής. Οι υπόλοιπες επιλογές του
 * φύλλου επιβίωσαν — τέσσερις είναι καρτέλες του [AddPlaylistScreen] και η
 * εισαγωγή EPG υπάρχει στην καρτέλα EPG — αυτή εδώ όχι.
 *
 * ΔΕΝ ΔΙΑΓΡΑΦΗΚΕ ΕΠΙΤΗΔΕΣ: η απόφαση αν η λειτουργία φεύγει οριστικά ή αν
 * αποκτά νέα πόρτα (Ρυθμίσεις, ή πέμπτη καρτέλα στην προσθήκη πηγής) είναι του
 * κατόχου και εκκρεμεί. Να αφαιρεθεί ολόκληρος μαζί με αυτό το σχόλιο μόλις
 * απαντηθεί, ή να ξανασυνδεθεί.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
