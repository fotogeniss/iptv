...
object TmdbClient {
    ...
    fun episodeMeta(rawTitle: String, yearHint: String, season: Int): Map<Int, EpisodeMeta> {
        ...
        synchronized(lockFor(cacheKey)) {
            ...
            // Στον δίσκο μόνο ό,τι έχει περιεχόμενο: ένα κενό αποτέλεσμα μπορεί
            // να οφείλεται σε αποτυχία δικτύου και δεν πρέπει να μονιμοποιηθεί.
            if (result.isNotEmpty()) saveEpisodesDisk(cacheKey, result)
            return result
        }
    }
